#!/usr/bin/env bash
# Stage 2: build the API container image and push it to Stage 1's ACR
# (deployment/azure/dev/dev03-container-registry.sh). Uses `az acr build`
# (ACR Tasks) rather than local `docker build`/`docker push`: the build runs
# inside the registry's own build service using the operator's Azure RBAC on
# the registry, so it works even though dev03 created the ACR with
# --admin-enabled false (matches the pattern already used in production by
# deployment/azure/scripts/az08b-build-and-deploy-west-application.sh).
set -Eeuo pipefail
...
DOCKERFILE_RELATIVE="deployment/containers/api/Dockerfile"

STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOG="${LOG_DIR}/dev05-build-push-api-image-${STAMP}.log"

{
    section "DEV-05 - Pulse DEV API image build & push"
    echo "TIME=$(date -u -Is)"
    echo "Resource group: $RG_DEV"
    echo "Registry: $ACR_NAME"
    echo "Repository: $API_REPOSITORY"
    ...
    az acr build \
        ...
        --image "${API_REPOSITORY}:dev-latest" \
        --only-show-errors \
        "$REPO_ROOT"
    ...
    IMAGE_DIGEST="$(az acr repository show --name "$ACR_NAME" --image "${API_REPOSITORY}:${IMAGE_TAG}" --query digest --output tsv)"

    [ -n "$ACR_LOGIN_SERVER" ] || fail "Could not resolve login server for ACR $ACR_NAME."
    [ -n "$IMAGE_DIGEST" ] || fail "Could not resolve pushed image digest for ${API_REPOSITORY}:${IMAGE_TAG}."

    # Deploy scripts pin to this digest rather than a mutable tag, so a later
    # `dev06` run always deploys exactly what this run built and verified,
    # even if `dev-latest` moves again before dev06 runs.
    API_IMAGE="${ACR_LOGIN_SERVER}/${API_REPOSITORY}@${IMAGE_DIGEST}"

    echo "ACR_LOGIN_SERVER=$ACR_LOGIN_SERVER"
    echo "API_IMAGE=$API_IMAGE"

    record_config ACR_LOGIN_SERVER "$ACR_LOGIN_SERVER"
    record_config API_IMAGE_REPOSITORY "$API_REPOSITORY"
    record_config API_IMAGE_TAG "$IMAGE_TAG"
    record_config API_IMAGE_DIGEST "$IMAGE_DIGEST"
    record_config API_IMAGE "$API_IMAGE"

    section "Validation"

    az acr repository show --name "$ACR_NAME" --image "${API_REPOSITORY}:${IMAGE_TAG}" \
        --query '{image:name,digest:digest,createdTime:createdTime}' \
        --output table

    section "DEV-05 complete"
